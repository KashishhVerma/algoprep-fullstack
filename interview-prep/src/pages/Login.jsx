    ...
    <div className="gradient-mesh" style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div className="animate-fade-up" style={{ width: "100%", maxWidth: 420 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <p style={{ color: "var(--text-muted)", fontSize: 13, marginTop: 4 }}>Your DSA interview companion</p>
        </div>

        <div className="card" style={{ padding: 28 }}>
          <h2 style={{ margin: "0 0 6px", fontSize: 18, fontWeight: 600 }}>Welcome back</h2>
          <p style={{ color: "var(--text-muted)", fontSize: 13, margin: "0 0 24px" }}>Sign in to continue your prep</p>

          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 6, fontWeight: 500 }}>EMAIL</label>
              <input className="input-dark" style={{ width: "100%" }} type="email" placeholder="you@example.com" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required />
            </div>
            <div style={{ marginBottom: 20 }}>
              <label style={{ fontSize: 12, color: "var(--text-muted)", display: "block", marginBottom: 6, fontWeight: 500 }}>PASSWORD</label>
              <input className="input-dark" style={{ width: "100%" }} type="password" placeholder="••••••••" value={form.password} onChange={e => setForm(p => ({ ...p, password: e.target.value }))} required />
            </div>
            ...
            <button className="btn-primary" style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, fontSize: 14 }} disabled={loading}>
              {loading ? <><Loader2 size={15} style={{ animation: "spin 1s linear infinite" }} /> Signing in...</> : "Sign In"}
            </button>
          </form>

          <p style={{ textAlign: "center", marginTop: 20, fontSize: 13, color: "var(--text-muted)" }}>
            No account? <Link to="/register" style={{ color: "var(--accent-green)", textDecoration: "none", fontWeight: 600 }}>Create one</Link>
          </p>
        </div>

        <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(59,158,255,0.06)", border: "1px solid rgba(59,158,255,0.15)", borderRadius: 8, fontSize: 12, color: "var(--text-muted)", textAlign: "center" }}>
          💡 Make sure your backend is running on <strong style={{color:"var(--accent-blue)"}}>localhost:5000</strong>
        </div>
      </div>
